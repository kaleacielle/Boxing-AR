...
void Update(){if(head==null)return;Move(ih,head.position);Move(inck,neck.position);Move(ils,ls.position);Move(ile,lf.position);Move(ilw,lh.position);Move(irs,rs.position);Move(ire,rf.position);Move(irw,rh.position);D(bn,head.position,neck.position);D(bs,ls.position,rs.position);D(blu,ls.position,la.position);D(bll,lf.position,lh.position);D(bru,rs.position,ra.position);D(brl,rf.position,rh.position);}
